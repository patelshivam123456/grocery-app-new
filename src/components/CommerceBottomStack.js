...
export default function CommerceBottomStack({ navigation, showUnlock = true, onCartPress,bottomOffset = 0 }) {
  ...
  const unlockBottom = (hasAction ? 56 : 0) + bottomOffset;
    ...
    >
      ...
      <View style={styles.unlockCopy}>
        <Text style={styles.unlockTitle}>
          {unlocked ? "Free delivery unlocked" : "Unlock free delivery"}
        </Text>

        <Text style={styles.unlockSub}>
          {unlocked
            ? "Delivery charge is on us"
            : `Shop for ₹${remaining}`}
        </Text>
...
/>
      ...
      : */}
      {hasOrder? (
        <View style={{width:"70%",alignSelf:"center"}}><OrderStatusBottomBar navigation={navigation} bottomOffset={bottomOffset} /></View>
      ) : count > 0 ? (
        <FloatingCartPill navigation={navigation} bottomOffset={bottomOffset} onPress={onCartPress} style={styles.cartPill} />
      ) : null}
...
unlockBarRow: {
  ...
  unlockBar: { position: "absolute", left: 12, right: 12, minHeight: 54, borderRadius: 10, backgroundColor: "#202833", borderWidth: 1, borderColor: "#303A48", flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 20, zIndex: 18 },
